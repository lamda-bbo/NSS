...
class GridArchive(ribs.archives.GridArchive):
    ...
    def get_mop2_selected_id(self, n: Optional[int] = None) -> List[int]:
        objectives: np.ndarray = self._objective_values[self._occupied_indices_cols]
        behaviors: np.ndarray = -self._behavior_values[self._occupied_indices_cols]

        survivors = []

        fronts = DominatorNumba.mop2_sort(-behaviors, -objectives)
    ...
    def get_nslc_fronts(self, k: int, n: Optional[int] = None) -> List[int]:
        objectives: np.ndarray = self._objective_values[self._occupied_indices_cols]
        behaviors: np.ndarray = self._behavior_values[self._occupied_indices_cols]
    ...
    def get_nss_fronts(self) -> List[List[int]]:
        objectives: np.ndarray = self._objective_values[self._occupied_indices_cols]
        behaviors: np.ndarray = self._behavior_values[self._occupied_indices_cols]

        fronts = DominatorNumba.non_surrounded_dominated_sort(self._behavior_dim, behaviors, objectives)
